...
from model.dataset.augmentation import generate_unsup_data
from model.dataset.builder import get_loader,get_loader_new
from model.models.model_helper import ModelBuilder, Segmentation_model_Point
from model.utils.dist_helper import setup_distributed

from model.models.vit_transformer_ import VisionTransformer
from model.models.vit_transformer import CONFIGS as CONFIGS_ViT_seg


from model.utils.loss_helper import (compute_contra_memobank_loss, compute_unsupervised_loss, get_criterion, )
from model.utils.lr_helper import get_optimizer, get_scheduler
from model.utils.utils import (AverageMeter, get_rank, get_world_size, init_log, intersectionAndUnion, label_onehot, load_state, set_random_seed)

parser = argparse.ArgumentParser(description="Semi-Supervised Semantic Segmentation")
parser.add_argument("--config", type=str, default="path/config.yaml")
...
def main():
    ...
    modules_back = [model.transformer]
    modules_bo = [model.decoder]
    modules_head = [model.end]
    ...
    train_loader_sup, val_loader = get_loader_new(cfg, seed=seed)
    ...
    model_teacher = VisionTransformer(config_vit, img_size=args.img_size, num_classes=config_vit.n_classes)#Segmentation_model_Point(filters=32, in_channels=3) #ModelBuilder(cfg["net"])

    model_teacher = model_teacher.cuda()
    model_teacher = torch.nn.parallel.DistributedDataParallel(model_teacher,device_ids=[local_rank], output_device=local_rank, find_unused_parameters=True )
    ...
    if cfg["saver"].get("auto_resume", False):
        ...
        if not os.path.exists(lastest_model):
            ...
            _, _ = load_state( lastest_model, model_teacher, optimizer=optimizer, key="teacher_state" )
    ...
    lr_scheduler = get_scheduler( cfg_trainer, len(train_loader_sup), optimizer_start, start_epoch=last_epoch )
    ...
    # build prototype
    prototype = torch.zeros( (cfg["net"]["num_classes"], cfg["trainer"]["contrastive"]["num_queries"],1,256,)).cuda()
    ...
    for epoch in range(last_epoch, cfg_trainer["epochs"]):
        ...
        if cfg_trainer["eval_on"]:
            ...
            if rank == 0:
                state = { "epoch": epoch + 1, "model_state": model.state_dict(), "optimizer_state": optimizer.state_dict(), "teacher_state": model_teacher.state_dict(), "best_miou": best_prec, }
                if prec > best_prec:
                    best_prec = prec
                    torch.save( state, osp.join(cfg["saver"]["snapshot_dir"], "ckpt_best.pth") )
                torch.save(state, osp.join(cfg["saver"]["snapshot_dir"], "ckpt.pth"))
                logger.info( "\033[31m * Currently, the best val result is: {:.2f}\033[0m".format( best_prec * 100 ) )
                tb_logger.add_scalar("mIoU val", prec, epoch)


def train( model,model_teacher,optimizer,lr_scheduler,sup_loss_fn,loader_l,epoch,tb_logger,logger,memobank,queue_ptrlis,queue_size,):
    global prototype
    ...
    for step in range(len(loader_l)):
        ...
        if epoch < cfg["trainer"].get("sup_only_epoch", 1):
            ...
            if "aux_loss" in cfg["net"].keys():
                ...
                sup_loss = F.cross_entropy(pred, label_l, ignore_index=255)
            model_teacher.train()
            ...
            model_teacher.eval()#
            pred_u_teacher = model_teacher(image_l)["pred"]
            ...
            if np.random.uniform(0, 1) < 0.5 and cfg["trainer"]["unsupervised"].get("apply_aug", False):
                image_u_aug, label_u_aug, logits_u_aug = generate_unsup_data(  image_l, label_u_aug.clone(), logits_u_aug.clone(), mode=cfg["trainer"]["unsupervised"]["apply_aug"], )
            ...
            if "aux_loss" in cfg["net"].keys():
               ...
               sup_loss = F.cross_entropy(pred_l_large, label_l.clone(),ignore_index=255)
            ...
            with torch.no_grad():
                ...
                prob_l_teacher, prob_u_teacher = (prob_all_teacher[:num_labeled], prob_all_teacher[num_labeled:], )
                pred_u_teacher = pred_all_teacher[num_labeled:]
                pred_u_large_teacher = F.interpolate(pred_u_teacher, size=(h, w), mode="bilinear", align_corners=True)
            ...
            unsup_loss = ( compute_unsupervised_loss(pred_u_large,label_u_aug.clone(), drop_percent, pred_u_large_teacher.detach(),) * cfg["trainer"]["unsupervised"].get("loss_weight", 1) )


            contra_flag = "none"
            if cfg["trainer"].get("contrastive", False):
                cfg_contra = cfg["trainer"]["contrastive"]
                contra_flag = "{}:{}".format( cfg_contra["low_rank"], cfg_contra["high_rank"] )
                alpha_t = cfg_contra["low_entropy_threshold"] * (1 - epoch / cfg["trainer"]["epochs"])
                ...
                with torch.no_grad():
                    ...
                    if cfg_contra.get("negative_high_entropy", True):
                        ...
                        contra_flag += " low"
                        high_mask_all = torch.cat(((label_l.unsqueeze(1) != 255).float(),torch.ones(logits_u_aug.shape).float().unsqueeze(1).cuda(), ),)
                    ...
                    label_u_small = F.interpolate(label_onehot(label_u_aug, cfg["net"]["num_classes"]),size=pred_all.shape[2:], mode="nearest",)
                ...
                contra_loss = ( contra_loss / world_size* cfg["trainer"]["contrastive"].get("loss_weight", 1))
...
def validate(  model,data_loader, epoch,logger,):
    model.eval()
