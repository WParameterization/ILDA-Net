...
def build_voc_semi_loader(split, all_cfg, seed=0):
    ...
    workers = cfg.get("workers", 2)
    batch_size = cfg.get("batch_size", 1)
    n_sup = 2160-cfg.get("n_sup", 10582)
...
def build_voc_semi_loader_new(split, all_cfg, seed=0):
    cfg_dset = all_cfg["dataset"]
    ...
    if split == "val":
        # build sampler
        sample = DistributedSampler(dset)
        loader = DataLoader(  dset, batch_size=batch_size, num_workers=workers,sampler=sample, shuffle=False,pin_memory=True, drop_last=True)
        return loader

    else:
        # build sampler for unlabeled set
        # data_list_unsup = cfg["data_list"].replace("labeled.txt", "unlabeled.txt")
        # dset_unsup = voc_dset(cfg["data_root"], data_list_unsup, trs_form_unsup, seed, n_sup, split)

        sample_sup = DistributedSampler(dset)
        loader_sup = DataLoader( dset, batch_size=batch_size,num_workers=workers,sampler=sample_sup, shuffle=False, pin_memory=True,drop_last=True, )
        # sample_unsup = DistributedSampler(dset_unsup)
        # loader_unsup = DataLoader( dset_unsup, batch_size=batch_size, num_workers=workers, sampler=sample_unsup,shuffle=False, pin_memory=True, drop_last=True, )
        return loader_sup#, loader_unsup