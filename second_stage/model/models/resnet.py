...
model_urls = {
    ...
    "resnet50": "/home/jidian/sdb1/jinlei/U2PL/prtain/resnet50.pth",
    "resnet101": "/home/jidian/sdb1/jinlei/U2PL/prtain/resnet101.pth",
...
class ResNet(nn.Module):
    ...
    def _make_layer( self, block, planes, blocks, stride=1, dilate=False, multi_grid=False):
        ...
        layers = []
        layers.append(block(self.inplanes, planes,stride, downsample, self.groups, self.base_width, previous_dilation * grids[0], norm_layer,))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block( self.inplanes,  planes, groups=self.groups, base_width=self.base_width,dilation=self.dilation * grids[i],norm_layer=norm_layer, ))
        return nn.Sequential(*layers)

    def forward(self, x):

        x = self.relu(self.bn1(self.conv1(x)))#resnet是第一个输入是三个通道啊
        x = self.maxpool(x)
