...
def get_parser():
    ...
    parser.add_argument(  "--base_size", type=int, default=2, help="based size for scaling"   )
    ...
    parser.add_argument(  "--model_path", type=str,default="checkpoints/psp_best.pth",help="evaluation model path",)
    parser.add_argument(
        "--save_folder",
        type=str,
...
def main():
    ...
    if "cityscapes" in data_root:
        ...
        for line in open(f_data_list, "r"):
            arr = [
                "JPEGImages/{}".format(line.strip()),
                "SegmentationClass/{}".format(line.strip()),
            ]
            arr = [os.path.join(data_root, item) for item in arr]
            data_list.append(arr)
    ...
    #model  = Segmentation_model_Point(filters=32, in_channels=3)#model = ModelBuilder(cfg["net"])
...
def valiadte_whole(
    ...
    for i, (input_pth,label_path) in enumerate(data_list):
        ...
        # print(gray1.shape)
        # print(gray1.ndim)
        intersection, union, target = intersectionAndUnion(gray1, label, classes)
        intersection_meter.update(intersection)
        union_meter.update(union)
