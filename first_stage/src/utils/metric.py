...
def dice_coef_multilabel(y_true, y_pred, numLabels=8, channel='channel_first'):
    """
...
def metrics3(img_gt, img_pred, apply_hd=False, apply_asd=False):
    """
    ...
    res = {}
    class_name = ["底座", "大圆柱齿轮", "大圆锥齿轮"]#["底座", "大圆柱齿轮", "大圆锥齿轮", "小圆锥齿轮","大轴","小轴","小圆柱齿轮","头端盖","其他"]
    # Loop on each classes of the input images
    for c, cls_name in zip([1, 2, 3], class_name) :
...
def metrics2(img_gt, img_pred, apply_hd=False, apply_asd=False):
    ...
    if img_gt.ndim != img_pred.ndim:
        raise ValueError("The arrays 'img_gt' and 'img_pred' should have the  same dimension, {} against {}".format(img_gt.ndim, img_pred.ndim))
    res = {}
    class_name = ["底座", "大圆柱齿轮", "大圆锥齿轮", "小圆锥齿轮","大轴","小轴","小圆柱齿轮","头端盖","其他"]
    # Loop on each classes of the input images
    for c, cls_name in zip([1, 2,3,4,5,6,7,8,9], class_name) :
...
def metrics4(img_gt, img_pred, apply_hd=False, apply_asd=False):
    """
    evaluate the models on mmwhs data in batches
    :param img_gt: the ground truth
    :param img_pred: the prediction
    ...
    class_name = ["底座", "大圆柱齿轮"]
    # Loop on each classes of the input images
    for c, cls_name in zip([1, 2], class_name) :
