...
class ModelCheckPointCallback:
    ...
    def __init__(self, mode="min", best_model_name=None, save_best=False, entire_model=False, save_last_model=False, model_name="../weights/model_checkpoint.pt", n_epochs=200):
        """
    ...
    def step(self, monitor, model, epoch, optimizer=None):
        ...
        if self._save_best:
            ...
            if epoch == 1 or better:
                self.best_result = monitor
                self.epoch = epoch
                save({'epoch': epoch, 'model_state_dict': to_save, 'optimizer_state_dict': opt_to_save}, self.best_model_name)
        ...
        if self.save_last_model and (epoch == self.n_epochs):
            ...
            if self.entire_model:
                ...
                if optimizer is not None:
                    opt_to_save = optimizer.state_dict()
            save({'epoch': epoch, 'model_state_dict': to_save, 'optimizer_state_dict': opt_to_save}, self.model_name)
