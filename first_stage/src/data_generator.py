...
def augmentation(image, mask=None,line=None):
    ...
    if mask is None and line is None:
        ...
        if image.ndim == 4:
            mask = np.array(mask)
            line=np.array(line)
            aa=np.concatenate([mask,line],axis=3)

            image_heavy ,label_heavy= seq(images=image,segmentation_maps=aa.astype(np.int32))
...
class DataGenerator_PointNet:
    ...
    def get_image_paths(self, id,id1):
        ...
        return img_path, mask_path,line_path,line_path_1
    ...
    def __next__(self):
        ...
        for _id in ids_train_batch.values:
            img_path, mask_path,line_path,line_path_1 = self.get_image_paths(id=_id,id1=_id)
        ...
        return images, masks,lines,lines1
